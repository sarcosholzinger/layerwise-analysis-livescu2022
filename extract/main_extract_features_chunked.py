...
"""
HuBERT Feature Extraction Pipeline - Chunked Processing

This script manages the extraction of HuBERT features from audio files in a chunked manner,
optimized for SLURM environments and large datasets.

PURPOSE:
    Extracts HuBERT features from audio files by processing them in manageable chunks,
    allowing for better resource management and the ability to resume interrupted jobs.

FUNCTIONALITY:
    1. Audio File Management:
       - Scans directories for .wav and .flac files
       - Groups files into configurable chunk sizes
       - Tracks completed files to enable resumption
    
    2. Chunk Processing:
       - Creates temporary directories for each chunk
       - Maintains original directory structure via symlinks
       - Processes chunks sequentially with error handling
    
    3. Resource Management:
       - Configurable GPU usage (single or multi-GPU)
       - Memory-efficient processing via chunking
       - Automatic cleanup of temporary files
    
    4. Progress Tracking:
       - Maintains processing statistics
       - Allows resumption from specific chunks
       - Provides detailed progress reporting

USAGE:
    python extract/main_extract_features_chunked.py \
...
STATUS:
    ACTIVE - This is a primary entry point for the feature extraction pipeline.
    (extract/run_extract_features-HuBERT.py) -- although this is the main entry point, it hasn't been tested yet!
    
    INTEGRATION:
    - Directly used in the main analysis workflow
    - Required for preparing data for layerwise analysis
    - Essential for temporal analysis pipeline
    
    DEPENDENCIES:
    - extract_features-HuBERT.py (core extraction logic)
    - HuBERT model from HuggingFace
    - SLURM environment (optional but recommended)
...
def run_chunk(chunk_files, chunk_id, args, base_audio_dir):
    ...
    try:
        ...
        cmd = [
            sys.executable, "extract_features-HuBERT.py",
